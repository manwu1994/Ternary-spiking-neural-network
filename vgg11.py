...
class SNN_VGG11_BNTT(nn.Module):
    def __init__(self, num_steps, leak_mem=1.0, img_size=32,default_threshold = 1.0,num_cls=100):
        ...
        self.conv7 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias=bias_flag)
        self.bntt7 = nn.BatchNorm2d(128, eps=1e-4, momentum=0.8, affine=affine_flag) 
        self.conv8 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias=bias_flag)
        self.bntt8 = nn.BatchNorm2d(128, eps=1e-4, momentum=0.8, affine=affine_flag)
        self.conv9 = nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1, bias=bias_flag)
        self.bntt9 = nn.BatchNorm2d(256, eps=1e-4, momentum=0.8, affine=affine_flag)
        ...
        self.threshold9=nn.Parameter(torch.tensor(default_threshold))
    ...
    def forward(self, inp):
        ...
        mem_conv7 = torch.zeros(batch_size, 128, self.img_size, self.img_size).cuda()
        mem_conv8 = torch.zeros(batch_size, 128, self.img_size, self.img_size).cuda()
        mem_conv9 = torch.zeros(batch_size, 256, self.img_size, self.img_size).cuda()
        ...
        for t in range(self.num_steps):
            ...
            out_prev3 = out.clone()
            out_prev33=out_prev3+out_prev2
            ...
            #self.conv6.weight.data =torch.tanh(self.conv6.weight.data)
            ...
            #rst[mem_thr > 0] = self.threshold
            mem_conv6 = mem_conv6 - rst
            out_prev6 = out.clone()
            out_prev66=out_prev6+out_prev5
            #self.conv7.weight.data =torch.tanh(self.conv7.weight.data)
            ...
            out_prev7 = out.clone()
            out_prev77=out_prev7+out_prev6
            
            mem_conv8 = self.leak_mem * mem_conv8 + self.bntt8(self.conv8(out_prev77))
            mem_thr = (mem_conv8 / self.threshold8) - 1.0
            out = self.spike_fn(mem_thr)
            rst = self.threshold8* (mem_thr>0).float()
            #rst = torch.zeros_like(mem_conv7).cuda()
            #rst[mem_thr > 0] = self.threshold7
            mem_conv8 = mem_conv8 - rst
            out_prev8 = out.clone()
            out_prev88=out_prev8+out_prev7
            
            mem_conv9 = self.leak_mem * mem_conv9 + self.bntt9(self.conv9(out_prev88))
            mem_thr = (mem_conv9 / self.threshold9) - 1.0
            out = self.spike_fn(mem_thr)
            rst = self.threshold9* (mem_thr>0).float()
            #rst = torch.zeros_like(mem_conv7).cuda()
            #rst[mem_thr > 0] = self.threshold7
            mem_conv9 = mem_conv9 - rst
            out_prev10 = out.clone()
            
            
            out_pool3=self.pool3(out_prev10)
